...
class ACDNet(nn.Module):
    def __init__(self, input_length, n_class, sr, ch_conf=None):
        ...
        if self.ch_config is None:
            self.ch_config = [channels, channels*8, channels*4, channels*8, channels*8, channels*16, channels*16, channels*32, channels*32, channels*64, channels*64, emb_len];
        else:
            del self.ch_config[-1];
            self.ch_config.append(emb_len);
        ...
        fcn = nn.Linear(self.ch_config[-1], n_class);
        ...
        tfeb_modules.append(nn.Dropout(0.25));
...
def GetMicroACDNetModel(input_len=30225, nclass=50, sr=20000, channel_config=None):
    channel_config = [7, 20, 10, 14, 22, 31, 35, 41, 51, 67, 69, 48];
    net = ACDNet(input_len, nclass, sr, ch_conf=channel_config);
    return net;

# if __name__ == '__main__':
#     ch_config = [8, 64, 32, 64, 64, 128, 128, 256, 256, 512, 512, 50];
#     input_len = 30225;
#     sr = 20000;
#     net = GetModel(input_len, 50, sr, ch_config);
