...
if __name__=='__main__':
    ...
    for ds in datasets:
        ...
        # data = np.load(os.path.join(opt.data, opt.dataset, 'raw-data/train_small.npz'), allow_pickle=True);
        # X = data['x'];
        # Y = [int(idx) for idx in data['y']];
        # opt.batchSize=len(X);
        # print('Train- BatchSize: {}, Epochs: {}'.format(opt.batchSize, opt.nEpochs));
        # aug_data_dir = os.path.join(data_dir, 'aug_data', 'train_small');
        ...
        # trainX, trainY = trainGen.get_batch(0);
        # np.savez_compressed('{}/train_small'.format(data_dir), x=trainX, y=trainY);
        #
        # print('==========Finished training data generation================');
        # break;
        #Generate Validation Data
        start_time = time.perf_counter();
        data = np.load(os.path.join(opt.data, opt.dataset, 'raw-data/val_small.npz'), allow_pickle=True);
        X = data['x'];
        Y = [int(idx) for idx in data['y']];
        opt.batchSize=len(X);
        ...
        np.savez_compressed('{}/val_small'.format(data_dir), x=valX, y=valY);
        ...
        #Generate Test Data
        start_time = time.perf_counter();
        data = np.load(os.path.join(opt.data, opt.dataset, 'raw-data/test_small.npz'), allow_pickle=True);
        X = data['x'];
        Y = [int(idx) for idx in data['y']];
        opt.batchSize=len(X);
        ...
        np.savez_compressed('{}/test_small'.format(data_dir), x=testX, y=testY);
        ...
        #Generate Test Data
        data = np.load(os.path.join(opt.data, opt.dataset, 'raw-data/unlblpool_small.npz'), allow_pickle=True);
        X = data['x'];
        Y = [int(idx) for idx in data['y']];
        opt.batchSize=len(X);
        ...
        np.savez_compressed('{}/unlblpool_small'.format(data_dir), x=unlblX, y=unlblY);
