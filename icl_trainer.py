...
class Trainer:
    ...
    def train(self):
        ...
        net = None;
        if self.opt.modelPath != '':
            net_path = self.opt.modelPath;
            print(self.opt.modelPath)
    ...
    def label_new_batch(self):
        ...
        q_idxs = random.sample(list(unlbl), k=self.opt.newLabelsPerLoop);
...
if __name__ == '__main__':
    ...
    opt.initLR = 0.0001; #The trained model had the highest accuracy on LR: 0.001
    opt.schedule = [0.8];
    opt.warmup = 0;
    opt.batchSize = 16;
    opt.sr = 20000;
